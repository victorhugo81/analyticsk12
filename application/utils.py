...
def _get_fernet(encryption_key: str) -> Fernet:
    """Derive a Fernet instance from the app's DATA_ENCRYPTION_KEY.

    Deliberately not SECRET_KEY — data-at-rest encryption and session/CSRF signing are
    different trust boundaries and use different keys, so a leak of one doesn't
    compromise the other and SECRET_KEY can be rotated without breaking stored data.
    """
    key = base64.urlsafe_b64encode(hashlib.sha256(encryption_key.encode()).digest())
    return Fernet(key)


def encrypt_mail_password(password: str, encryption_key: str) -> str:
    """Encrypt a plain-text value (SMTP/FTP password, user email) for storage.

    `encryption_key` should be app.config['DATA_ENCRYPTION_KEY'] — see _get_fernet.
    """
    if not password:
        return ''
    return _get_fernet(encryption_key).encrypt(password.encode()).decode()


def hash_email(email: str, secret_key: str) -> str:
    """HMAC-SHA256 of the normalised email — used as the DB lookup/uniqueness token.

    Keyed by SECRET_KEY (not DATA_ENCRYPTION_KEY): this is a one-way lookup index, not
    data-at-rest encryption, so it stays with the app's general signing key.
    """
    return _hmac.new(
...
def decrypt_mail_password(encrypted: str, encryption_key: str) -> str:
    ...
    if not encrypted:
        return ''
    try:
        return _get_fernet(encryption_key).decrypt(encrypted.encode()).decode()
    except InvalidToken:
        return ''
