...
def create_app(config_name='default'):
    ...
    if config_name == 'production':
        ...
        if app.config.get('DATA_ENCRYPTION_KEY') == 'dev-data-key-insecure':
            raise RuntimeError(
                "DATA_ENCRYPTION_KEY must be set to a strong random value in production. "
                "Generate one with: python -c \"from cryptography.fernet import Fernet; "
                "print(Fernet.generate_key().decode())\" and set the DATA_ENCRYPTION_KEY "
    ...
    # Belt-and-suspenders alongside the config_name == 'production' guard above: that
    # guard only fires when config_name is literally the string 'production'. This one
    # fires whenever DEBUG ends up off (ProductionConfig, or 'default' now that it maps
    # to ProductionConfig too) but the secret key still looks like the dev fallback —
    # catching the same misconfiguration by its actual symptom rather than by name, so a
    # bare `create_app()` call with a still-unset SECRET_KEY doesn't fail silently.
    if not app.debug and app.config.get('SECRET_KEY') == 'dev-secret-key':
        import warnings
        warnings.warn(
            "SECRET_KEY is still the development fallback value even though DEBUG is "
            "off. Set a strong random SECRET_KEY in the environment before this instance "
            "serves real traffic — sessions and CSRF tokens are signed with it.",
            RuntimeWarning,
            stacklevel=2,
        )
    if not app.debug and app.config.get('DATA_ENCRYPTION_KEY') == 'dev-data-key-insecure':
        import warnings
        warnings.warn(
            "DATA_ENCRYPTION_KEY is still the development fallback value even though "
            "DEBUG is off. Set a strong random DATA_ENCRYPTION_KEY in the environment "
            "before this instance serves real traffic — user emails and SMTP/FTP "
        ...
        try:
            ...
            if org and org.mail_server:
                ...
                app.config['MAIL_PASSWORD'] = decrypt_mail_password(
                    org.mail_password or '', app.config['DATA_ENCRYPTION_KEY']
                )
