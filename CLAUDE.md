...
```bash
# Run dev server
uv run flask --app "main:create_app('development')" run
...
**Do not run `flask --app main.py run`** (without the factory argument) — the Flask CLI calls `create_app()` with no argument, which defaults to `ProductionConfig` (`config_name='default'`, by deliberate design — see `config.py`). `ProductionConfig` sets `SESSION_COOKIE_SECURE = True`, so on plain `http://localhost`, the browser silently discards the session cookie on every request. Symptom: any session-backed global filter (School Year, Site, Snap Date, Status) appears to reset/not update no matter what you pick, because `before_request`'s `set_session_defaults()` re-initializes a blank session on every single request.
...
# 2. Seed roles, default site, admin user, default graduation requirements (interactive)
...
| Model | Notes |
|---|---|
| `Student` | `status` is a computed `@property` from `enter_date`/`exit_date` — there is no `status` column in DB. Unique constraint is on `(student_id, schoolyr)`, **not** `student_id` alone — the same student can have one record per school year. `english_status` codes (`EO`/`IFEP`/`EL`/`RFEP`/`TBD`) are defined once in `forms.py` `_ENGLISH_STATUS_CHOICES` — no time-of-reclassification field exists. `email` and `ssid` are encrypted at rest (`email_enc`/`ssid_enc`, `cryptography.fernet`) behind Python properties of the same name — reading/writing `student.email`/`student.ssid` works exactly like a normal column everywhere except inside a SQLAlchemy query, where the property isn't queryable; use `Student.ssid_hash` (a deterministic HMAC blind index — see `utils.hash_ssid`) for any `.filter()`/`.join()`/`.group_by()`/`.in_()` that used to reference `Student.ssid`. `demographics.csv`'s district-local `student_id` (the `(student_id, schoolyr)` unique key) is **not** encrypted — only the CALPADS statewide `ssid` is. |
...
| `Absence` | Linked to student via `ssid` string (not FK), site via `site_id` FK. No `abs_abbr` column. `ssid` is encrypted at rest exactly like `Student.ssid` (`ssid_enc` + `ssid_hash` blind index, same property pattern) — since there's no FK, every Student↔Absence cross-reference in `routes.py` (dozens of dashboards) joins/groups on `Absence.ssid_hash == Student.ssid_hash`, never on decrypted values. Indexed on `ssid_hash`, `(school_yr, site_id)`, and `(school_yr, ssid_hash)` — the table is 270K+ rows district-wide and every dashboard query filters by `school_yr` then groups by `ssid_hash` or `site_id`, so these were a real perf fix, not routine. |
| `Incident` | Linked to student via `sisid` string (not FK); `site` is stored as the site acronym string. **`sisid` is intentionally left unencrypted** (unlike `Student.ssid`/`Absence.ssid`) — it's just a denormalized copy of the SSID used solely as this join key, not a system of record for it. Cross-referencing it against an encrypted `Student.ssid` means hashing the plaintext `sisid` with `utils.hash_ssid()` and comparing to `Student.ssid_hash`, not decrypting the Student side — see `discipline()`/`incidents()`/`_suspension_rate()` in `routes.py` for the pattern (bounded to the students who actually have incidents, not a district-wide decrypt). |
...
| `Site` | Has `site_name`, `site_acronyms`, `site_cds`, `site_city`, `site_state`, `site_zip`, and principal contact fields (`principal_first_name`, `principal_last_name`, `principal_email`, `principal_phone`). `principal_email`/`principal_phone` are encrypted at rest (`principal_email_enc`/`principal_phone_enc`, same property pattern as `Student.email`) — not used in any `.ilike()`/`.filter()` anywhere, so this didn't require dropping a search feature. |
...
- **Student records are per school year**: `Student.student_id` is not globally unique. The composite unique constraint is `(student_id, schoolyr)`. Always scope student queries by `active_schoolyr` session filter. This applies to detail pages too, not just lists: `/student/<id>` and `/parent/<id>` are keyed by numeric PK, which pins them to one specific year's row/link. Both routes resolve to the row matching the active `active_schoolyr` filter (by the student's business `student_id`) when one exists, falling back to the originally-requested row otherwise — a new detail-style route reading `student.*` fields should follow the same pattern rather than trusting the PK's own `schoolyr` blindly.
- **Upload size limit**: `MAX_CONTENT_LENGTH = 100 MB` (set in `config.py`) to accommodate large absence exports.
- **Encrypted-field convention**: PII columns (`User.email`, `Parent.email`/`phone`, `Student.email`/`ssid`, `Teacher.email`, `Site.principal_email`/`principal_phone`) are Fernet-encrypted at rest (`utils.encrypt_mail_password`/`decrypt_mail_password`, keyed by `DATA_ENCRYPTION_KEY`) behind a Python `@property` of the plain field name backed by a `*_enc` column — reading/writing the property works exactly like a normal attribute (`student.email = 'x'`, `parent.phone`), including in `Model(**kwargs)` constructors and `obj=` form population, since those just go through `setattr`/`getattr`. The one thing a property can't do is appear in a SQLAlchemy query (`.filter()`, `.join()`, `.group_by()`, `.in_()`, `.ilike()`) — it isn't a mapped column. For fields only ever read/written on an already-loaded instance (`Teacher.email`, `Site.principal_*`) that's never an issue. For fields used in equality lookups at the SQL level (`Student.ssid`/`Absence.ssid`, the only link between those two tables since `Absence` has no FK to `Student`), there's also a deterministic HMAC blind-index column, `ssid_hash` (`utils.hash_ssid`, keyed by `SECRET_KEY` like `User.email_hash`) — use `Student.ssid_hash`/`Absence.ssid_hash` for any join/group/filter that used to reference `.ssid` directly, never the decrypted value. `Incident.sisid` is deliberately **not** encrypted (see the `Incident` model row) — cross-referencing it against `Student.ssid_hash` means hashing the plaintext `sisid` with `hash_ssid()`, not decrypting the Student side. None of this is searchable with `ilike()` against ciphertext — encrypting a field that was previously in a text-search filter (e.g. `Parent.email`, `Absence.ssid`) means dropping it from that search, matching how `/users` already excludes `User.email` from its own search for the same reason.
...
- **Continuation graduation track**: `Site.grad_track` (`'standard'` or `'continuation'`) picks which credit target applies to a student, since continuation schools typically require fewer credits. `GraduationRequirement.credits_required_continuation` is the per-subject override (NULL = same as `credits_required`); `Organization.grad_credits_required_continuation` is the manual-total fallback (NULL = same as `grad_credits_required`) used when `grad_auto_calculate_credits` is off. `_grad_credits_for(req_row, track)` in `routes.py` is the one place that resolves "which number applies" — `graduation_status()` looks up each student's site's `grad_track` and calls it per subject; don't recompute this fallback logic ad hoc elsewhere.
- **FTP sync treats every file as optional**: `ftp_bulk_upload_users()` tries `sites.csv`, `demographics.csv`, `staff.csv`, `courses.csv`, `master_schedule.csv`, `students_schedule.csv`, `parents.csv`, and `users.csv` in turn; a missing file (`ftplib.error_perm`, e.g. FTP 550) is caught and that file is skipped, not treated as a fatal error for the whole sync. If you add a new file to this list, wrap its `retrbinary` call the same way or one missing file will abort every other file's import.
